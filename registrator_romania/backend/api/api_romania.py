...
async def get_proxy_pool(start: bool = True, debug: bool = False, offset: int = 0):
    proxies_classes = [
        GeoNode(),
        FreeProxies(),
        FreeProxiesList(),
...
class APIRomania:
    ...
    async def get_recaptcha_token(self, proxy: str = None, use_proxy: bool = False, timeout: int = None):
        ...
        session = await self.get_session(with_proxy_if_exists=use_proxy, timeout=timeout)
    ...
    async def get_captcha_token(self, proxy: str = None, timeout: int = None):
        """Async get and return data for `g-recaptcha-response` field."""
    ...
    async def _get_disabled_days(self, year: int, month: int, tip_formular: int):
        session = await self.get_session()
    ...
    async def get_free_days(self, month: int, tip_formular: int, year: int = None):
        if not year:
            ...
            try:
                ...
                async with session.post(
                    ...
                    logger.debug(
                        ...
                        f"{url}: {raw.decode("utf-8", errors="ignore")}, "
                        f"proxy: {proxy}"
                    )
                    response = await resp.json(content_type=resp.content_type)
            except AIOHTTP_NET_ERRORS as e:
